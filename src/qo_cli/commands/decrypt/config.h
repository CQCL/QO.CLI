...
namespace Quantinuum::QuantumOrigin::Cli::Commands::Decrypt
{
    ...
    {
      public:
        explicit DecryptConfig(const std::string &configPath);
        explicit DecryptConfig(const YAML::Node &cfg);
        DecryptConfig(KeyParametersConfig keyParametersConfig, GeneralConfig generalConfig);
